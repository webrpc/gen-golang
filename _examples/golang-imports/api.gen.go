// example-api-service v1.0.0 3146da137395679454a09bf261102e571617df3a
// --
// Code generated by webrpc-gen@v0.36.2-0.20260321072159-3267f049abfb with ../../../gen-golang generator. DO NOT EDIT.
//
// webrpc-gen -schema=./proto/api.ridl -target=../../../gen-golang -out=./api.gen.go -pkg=main -server -client
package main
...
func WebRPCSchemaHash() string {
	return "3146da137395679454a09bf261102e571617df3a"
}
...
const AdminAPIPathPrefix = "/rpc/AdminAPI/"
...
func NewAdminAPIClient(addr string, client HTTPClient) AdminAPIClient {
	prefix := serviceURL(addr, AdminAPIPathPrefix)
	urls := [1]string{
		prefix + "Reset",
	}
...
func (c *adminAPIClient) Reset(ctx context.Context) error {
	err := doHTTPRequest(ctx, c.client, c.urls[0], nil, nil)
...
func (s *exampleAPIService) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/rpc/ExampleAPI/GetUser":
		handler = succinctHandler("GetUser", s.ExampleAPIServer.GetUser, s.sendErrorJSON)
	case "/rpc/ExampleAPI/ListUsers":
		handler = succinctHandler("ListUsers", s.ExampleAPIServer.ListUsers, s.sendErrorJSON)
	default:
...
func NewAdminAPIServer(svc AdminAPIServer, options ...*Options) *adminAPIService {
	server := &adminAPIService{
		AdminAPIServer: svc,
	}
	if len(options) > 0 && options[0] != nil {
		server.OnError = options[0].OnError
...
func (*adminAPIService) webRPCService() Service { return ServiceAdminAPI }

func (s *adminAPIService) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	defer func() {
		// In case of a panic, serve a HTTP 500 error and then panic.
		if rr := recover(); rr != nil {
			s.sendErrorJSON(w, r, ErrWebrpcServerPanic.WithCausef("%v", rr))
	...
	ctx = context.WithValue(ctx, HTTPResponseWriterCtxKey, w)
	ctx = context.WithValue(ctx, HTTPRequestCtxKey, r)
	ctx = context.WithValue(ctx, ServiceNameCtxKey, "AdminAPI")

	r = r.WithContext(ctx)

	var handler func(ctx context.Context, w http.ResponseWriter, r *http.Request)
	switch r.URL.Path {
	case "/rpc/AdminAPI/Reset":
		handler = s.serveResetJSON
	default:
		err := ErrWebrpcBadRoute.WithCausef("no webrpc method defined for path %v", r.URL.Path)
		s.sendErrorJSON(w, r, err)
		return
...
func (s *adminAPIService) serveResetJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	ctx = context.WithValue(ctx, MethodNameCtxKey, "Reset")

	// Call service method implementation.
	err := s.AdminAPIServer.Reset(ctx)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
...
func (s *adminAPIService) sendErrorJSON(w http.ResponseWriter, r *http.Request, rpcErr WebRPCError) {
	if s.OnError != nil {
		s.OnError(r, &rpcErr)
	}
...
const (
	...
	ServiceAdminAPI   Service = "AdminAPI"
)
...
const WebrpcHeaderValue = "webrpc@v0.36.2-0.20260321072159-3267f049abfb;gen-golang@unknown;example-api-service@v1.0.0"
