...
func WebRPCSchemaHash() string {
	return "bfaf85d9af004ef7f4011858164d748de7c28638"
}

const (
	Header_AuthToken = "authToken"
	Header_Role      = "role"
...
func (c *exampleServiceClient) GetUser(ctx context.Context, getUserRequest GetUserRequest) (*GetUserResponse, error) {
	{
		_headers := http.Header{}
		_headers.Set(Header_AuthToken, fmt.Sprint(getUserRequest.AuthToken))
	...
	resp, err := doHTTPRequest(ctx, c.client, c.urls[1], &_body, &out.Ret0)
	if resp != nil {
		cerr := resp.Body.Close()
		if err == nil && cerr != nil {
			err = ErrWebrpcRequestFailed.WithCausef("failed to close response body: %w", cerr)
...
func (c *exampleServiceClient) CreateUser(ctx context.Context, createUserRequest CreateUserRequest) (*CreateUserResponse, error) {
	{
		_headers := http.Header{}
		_headers.Set(Header_AuthToken, fmt.Sprint(createUserRequest.AuthToken))
		_headers.Set(Header_Role, fmt.Sprint(createUserRequest.Role))
		if _existingHeaders, ok := HTTPRequestHeaders(ctx); ok {
			for k := range _existingHeaders {
				for _, v := range _existingHeaders[k] {
					_headers.Add(k, v)
	...
	out := struct {
		Ret0 *CreateUserResponse
	}{}

	resp, err := doHTTPRequest(ctx, c.client, c.urls[2], &_body, &out.Ret0)
	if resp != nil {
		cerr := resp.Body.Close()
		if err == nil && cerr != nil {
			err = ErrWebrpcRequestFailed.WithCausef("failed to close response body: %w", cerr)
...
func (c *exampleServiceClient) DeleteUser(ctx context.Context, deleteUserRequest DeleteUserRequest) error {
	{
		_headers := http.Header{}
		_headers.Set(Header_AuthToken, fmt.Sprint(deleteUserRequest.AuthToken))
		_headers.Set(Header_UserID, fmt.Sprint(deleteUserRequest.UserID))
		if _existingHeaders, ok := HTTPRequestHeaders(ctx); ok {
			for k := range _existingHeaders {
				for _, v := range _existingHeaders[k] {
					_headers.Add(k, v)
...
func (s *exampleServiceService) serveGetUserJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	reqPayload.AuthToken = r.Header.Get(Header_AuthToken)

	// Call service method implementation.
	respPayload, err := s.ExampleServiceServer.GetUser(ctx, reqPayload)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
...
func (s *exampleServiceService) serveCreateUserJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	reqPayload.AuthToken = r.Header.Get(Header_AuthToken)
	reqPayload.Role = r.Header.Get(Header_Role)

	// Call service method implementation.
	respPayload, err := s.ExampleServiceServer.CreateUser(ctx, reqPayload)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
...
func (s *exampleServiceService) serveDeleteUserJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	var reqPayload DeleteUserRequest
	reqPayload.AuthToken = r.Header.Get(Header_AuthToken)
	if _v := r.Header.Get(Header_UserID); _v != "" {
		if _, _err := fmt.Sscan(_v, &reqPayload.UserID); _err != nil {
	...
	// Call service method implementation.
	err := s.ExampleServiceServer.DeleteUser(ctx, reqPayload)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
