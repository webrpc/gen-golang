...
curl -o schema/test.ridl -LJO https://raw.githubusercontent.com/webrpc/webrpc/$VERSION/tests/schema/api.ridl
...
$WEBRPC_GEN -schema=./test@$VERSION.ridl -target=golang -pkg=server -server -out=./server/server.gen.go
$WEBRPC_GEN -schema=./test@$VERSION.ridl -target=golang -pkg=client -client -out=./client/client.gen.go
